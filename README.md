...
In this project, you will be playing a role as an engineer with a partner engineer you choose to develop the system software for a hardware accelerator that your start-up company, Universal Chip of Riverside, is going to produce. The accelerator is a set of matrix multiplication units (MMUs) that can perform multiple $128\times128\times128$ matrix multiplications simultaneously. But as you could imagine, the hardware team is always falling behind and we cannot wait to start our development! Therefore, we have developed an emulated hardware accelerators, essentially a user program performing $128\times128\times128$ matrix multiplications. With multiple processes of this program running in your system, your system now have multiple emulated MMUs!
...
Now, the problem is back to you -- how to provide the appropriate system software support for these accelerators. You will need to implement a Linux kernel module, working as the device driver of the backend accelerators. The device driver receives commands from the user-program/library that are essentailly MMA requests broken down into $128\times128\times128$ ones and a synchronization request that reflects all local updates on the accelerators to the user program. However, there are a few challenges here: 
